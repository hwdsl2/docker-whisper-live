...
**在线试用：** [在 Colab 中打开](https://vpnsetup.net/whisper-live-notebook)。无需 Docker 或安装，直接在浏览器中运行。
...
```mermaid
graph LR
    D["📄 文档"] -->|向量化| E["Embeddings<br/>(文本 → 向量)"]
    E -->|存储| VDB["向量数据库<br/>(Qdrant, Chroma)"]
    A["🎤 实时音频"] -->|流式传输| WL["WhisperLive<br/>(实时 STT)"]
    WL -->|查询| E
