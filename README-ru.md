...
**1. Добавьте аутентификацию на прокси.** Сервер не имеет встроенного API-ключа. Для развёртывания с доступом из интернета добавьте Bearer-токен или basic auth на уровне обратного прокси — см. раздел «Добавление аутентификации на уровне прокси» в разделе [Использование обратного прокси](#использование-обратного-прокси). Привяжите WebSocket-порт (`9090`) к `127.0.0.1`, чтобы он был доступен только через прокси.
