...
- Связанные AI-сервисы: [Whisper (пакетный STT)](https://github.com/hwdsl2/docker-whisper/blob/main/README-ru.md), [Kokoro (TTS)](https://github.com/hwdsl2/docker-kokoro/blob/main/README-ru.md), [Embeddings](https://github.com/hwdsl2/docker-embeddings/blob/main/README-ru.md), [LiteLLM](https://github.com/hwdsl2/docker-litellm/blob/main/README-ru.md), [Ollama (LLM)](https://github.com/hwdsl2/docker-ollama/blob/main/README-ru.md), [Docling](https://github.com/hwdsl2/docker-docling/blob/main/README-ru.md), [MCP Gateway](https://github.com/hwdsl2/docker-mcp-gateway/blob/main/README-ru.md)
...
> **Временное примечание:** При включённой переменной `WHISPERLIVE_API_KEY` подключения WebSocket к порту `9090` сейчас завершаются ошибкой HTTP 500 из-за [проблемы в WhisperLive](https://github.com/collabora/WhisperLive/issues/532). Аутентифицированные REST-запросы к порту `8000` продолжают работать. За ходом исправления можно следить в [docker-whisper-live #1](https://github.com/hwdsl2/docker-whisper-live/issues/1).
>
> Если для работы WebSocket необходимо временно отключить встроенную аутентификацию, сначала привяжите оба порта к localhost и включите аутентификацию на обратном прокси. Не открывайте порты без аутентификации напрямую.
...
Все переменные необязательны. Новые установки с подключённым томом `/var/lib/whisper-live` автоматически генерируют API-ключ. Существующие установки без ключа остаются открытыми для обратной совместимости.
...
**1. Используйте API-ключ.** Новые установки с подключённым томом `/var/lib/whisper-live` автоматически генерируют API-ключ. Его можно посмотреть командой `docker exec whisper-live whisper_live_manage --showkey`; в скриптах используйте `docker exec whisper-live whisper_live_manage --getkey`. Существующие установки без ключа остаются открытыми для обратной совместимости; также можно задать `WHISPERLIVE_API_KEY` в env-файле вручную. REST-клиенты должны отправлять `Authorization: Bearer <key>`; WebSocket-клиенты могут отправлять тот же заголовок или добавить `?token=<key>` к URL. Из-за указанной выше проблемы встроенная аутентификация WebSocket по API-ключу временно не работает; аутентификация REST остаётся доступной. См. временное примечание в разделе [«Быстрый старт»](#быстрый-старт).

**2. Привяжите к localhost при использовании обратного прокси.** Замените `-p 9090:9090 -p 8000:8000` на `-p 127.0.0.1:9090:9090 -p 127.0.0.1:8000:8000` (или измените оба маппинга портов на их эквиваленты с `127.0.0.1:` в `docker-compose.yml`), чтобы незашифрованные порты нельзя было достичь напрямую снаружи хоста.
...
**4. Следите за уровнем журналирования.** При `WHISPERLIVE_LOG_LEVEL=DEBUG` текст транскрипции может попадать в журналы. На общих системах сохраняйте уровень `INFO` или выше.

**5. Ограничьте источники WebSocket на прокси.** WebSocket-соединения из ненадёжных браузерных источников можно заблокировать на уровне обратного прокси. Для nginx проверяйте `$http_origin` перед апгрейдом соединения; для Caddy используйте директиву `header` для проверки заголовка `Origin`.
...
<details>
<summary><strong>Добавление дополнительной аутентификации на уровне прокси</strong></summary>

Сервер поддерживает встроенную аутентификацию по API-ключу через `WHISPERLIVE_API_KEY`. Для развёртываний с доступом из интернета можно дополнительно добавить Bearer-токен или базовую аутентификацию на уровне обратного прокси. Пример с Caddy (`basicauth` защищает REST API):
